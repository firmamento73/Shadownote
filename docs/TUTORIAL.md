...
第二、三步有两种做法，产出完全一样，按你手头有没有 API key 选：

**A · 工作台自动跑（有 API key）** — 第一步拿到逐字稿后，直接丢进随影笔记的「工作台」，勾选「清理逐字稿」，点「一键生成笔记」，然后「在查看器中加载」。清理和生成一次跑完，`raw` 原文由程序按时间戳自动切好配上，**不用手工拼 JSON**，也不会出现模型把 JSON 写坏的情况。

> v1.1 之前工作台生成长逐字稿几乎必然失败（整篇被塞进一次请求、中途限流就前功尽弃、`raw` 还是空的）。这些已在 v1.1 修复，长课也能跑完了，建议优先走这条。

**B · 纯手工（没有 API key）** — 用任意大模型的网页聊天窗口完成第二、三步，一分钱不花。下面第二、三步写的就是这条路线，提示词可以直接复制。
...
**工作台一键生成跑到一半失败。**
状态栏会写明原因。若是 429（限流），程序已经会自动退避重试 3 次，仍失败说明配额确实用尽了，换个预设或稍后再跑。若提示「自检发现问题」，多半是模型编了时间戳——把「主题」填得更具体些重跑通常就好了。

**生成的笔记时间戳对不上，或者段落特别少。**
点「下载上次清理结果」检查清理后的逐字稿：确认每行是 `[mm:ss] 正文` 的形式、时间戳没被模型改动。清理步骤出问题会连累后面所有环节。
